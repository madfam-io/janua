...
|------------|---------|------|--------|
| **solarpunk-foundry** | Governance & Infrastructure | N/A | ✅ Ports standardized |
| **janua** | OAuth/OIDC Authentication | 4100 | ✅ Security hardened |
| **enclii** | Platform-as-a-Service | 4200 | ✅ Ready for integration |
...
#### ✅ FIXED: Port Allocation Standardized
...
#### 🟡 IMPORTANT: Redundant Docker Compose Files
...
if settings.ENABLE_BETA_ENDPOINTS:
    logger.warning("⚠️  SECURITY WARNING: Beta endpoints are ENABLED...")
...
| Section | Status |
|---------|--------|
| Environment Variables | ✅ Uses `JWT_SECRET_KEY` (correct) |
| Quick Start | ✅ Mentions `ADMIN_BOOTSTRAP_PASSWORD` env var |
| Port Allocation | ✅ Correct port table (4100-4199 block) |

### 2.3 Architecture Strengths

✅ **Comprehensive Auth Stack**
...
```python
async with db_manager.get_session() as session:  # ✅ Correct
...
- Self-hosted on Hetzner (~$55/month vs $2,220 Railway+Auth0)

### 3.2 Critical Findings

#### 🟢 READY: Janua OIDC Integration
...
#### ✅ FIXED: Port Configuration Standardized
...
- **Weeks 3-4**: Janua integration (⏳ Ready to deploy)
...
│  ┌──────────────────┐       OIDC        ┌────────────────┐  │
│  │     Janua        │◄─────────────────►│     Enclii     │  │
│  │   (Port 4100)    │   ID Token/JWKS   │   (Port 4200)  │  │
│  │                  │                    │                │  │
│  │ • OAuth/OIDC     │                    │ • PaaS Control │  │
│  │ • User Mgmt      │                    │ • Deployments  │  │
│  │ • Sessions       │                    │ • Builds       │  │
│  └────────┬─────────┘                    └───────┬────────┘  │
│           │                                      │           │
│           │        Orchestration                 │           │
...
│  │                                                           │
│  │  • PORT_ALLOCATION.md (canonical)                        │
│  │  • madfam.sh (orchestration)                             │
│  │  • Terraform configs                                      │
...
**Local Development**:
```
User → Janua (localhost:4100) → JWT issued
      → Enclii (localhost:4200) validates via:
...
**Production**:
```
User → api.janua.dev → JWT issued
      → api.enclii.dev validates via:
...
### 5.1 Completed Fixes (Priority: CRITICAL) ✅

| Fix | Status | Details |
|-----|--------|---------|
| Janua Allowed Hosts | ✅ Complete | Wildcard removed, k3s CIDR added, ports standardized |
| Solarpunk-Foundry Ports | ✅ Complete | docker-compose, env files, products.ts updated |
| Beta Endpoints | ✅ Complete | Gated behind ENABLE_BETA_ENDPOINTS flag |
| Enclii Port Config | ✅ Complete | Default port 4200, config.go updated |

### 5.2 Completed (Priority: MEDIUM) ✅

| Fix | Status | Details |
|-----|--------|---------|
| Cross-Repo CLAUDE.md Sync | ✅ Complete | Port tables added/verified in all CLAUDE.md files |
| Integration Test Docs | ✅ Complete | Created INTEGRATION_TESTING.md in solarpunk-foundry |
...
|-----------|--------|---------|
| Janua Dockerfile | ✅ Valid | Port 4100, health check, non-root user |
| Enclii Dockerfile | ✅ Fixed | Port 4200 (was 8001), health check updated |
| Janua Python | ✅ Compiles | app/main.py, app/config.py syntax verified |
| Enclii Go | ✅ Builds | switchyard-api compiles successfully |
...
| OIDC configuration | ✅ Pass | All endpoints configured correctly |
| JWKS endpoint | ⚠️ Empty | Expected - using HS256 (RS256 keys not configured) |
| Admin login | ✅ Pass | JWT issued for admin@madfam.io |
| Beta endpoints (404) | ✅ Pass | Feature flag working, returns 404 |
| API status | ✅ Pass | `v0.1.0`, `beta_endpoints:false` |
| Playwright verification | ✅ Pass | Screenshot captured |
...
| Health endpoint | ✅ Pass | `{"service":"switchyard-api","status":"healthy","version":"0.1.0"}` |
| Auth mode | ✅ Pass | Local JWT bootstrap mode |
| Playwright verification | ✅ Pass | Screenshot captured |
...
|-----|------|--------|----------|
| Dashboard | 4101 | ✅ Pass | Login form renders with "Sign in to Janua" |
| Docs | 4103 | ✅ Pass | Full documentation site with nav, guides, SDK docs |
| Admin | 4102 | ⚠️ Build Error | `@janua/feature-flags` module not found |
| Website | 4104 | ⚠️ Build Error | `@janua/typescript-sdk` module not found |
...
| Image | Tag | Status |
|-------|-----|--------|
| ghcr.io/madfam-org/janua-api | v0.1.0 | ✅ Pushed |
| ghcr.io/madfam-org/enclii-api | v0.1.0 | ✅ Pushed |

### 7.5 Docker Container Tests (macOS)

| Container | Status | Evidence |
|-----------|--------|----------|
| janua-api-test | ✅ Healthy | Port 4100, using host.docker.internal |
| janua-postgres | ✅ Healthy | Port 5432 |
| janua-redis | ✅ Healthy | Port 6379 |
...
| Version consistency | janua/apps/api/app/main.py | Changed 1.0.0 → 0.1.0 |
...
- Cost-effective infrastructure (~$55/month)
- Comprehensive dogfooding strategy
- Beta endpoints properly gated

**Remaining Items**:
- ✅ ~~Configure ghcr.io authentication for image push~~ (DONE)
- Set up npm.madfam.io private registry (blocks Admin/Website frontends)
- Deploy to K3s cluster
- Configure RS256 keys for production JWKS

**Status**: ✅ APIs Verified (Container + Dev) | ✅ Dashboard/Docs Working | ⚠️ Admin/Website Blocked (npm.madfam.io) | ⏳ Production Deployment Pending
