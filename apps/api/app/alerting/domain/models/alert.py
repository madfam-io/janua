...
class Alert:
    ...
    def escalate(self, escalation_reason: str) -> None:
        ...
        self._add_event("escalated", {
            "new_severity": self.severity.value,
            "reason": escalation_reason
        })
        
        logger.warning(f"Alert {self.alert_id} escalated to {self.severity.value}: {escalation_reason}")
...
class AlertAggregate:
    ...
    def can_be_escalated(self) -> bool:
        ...
        return (self._alert.status in [AlertStatus.TRIGGERED, AlertStatus.ACKNOWLEDGED] 
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if self._alert.metrics else None,
            "triggered_at": self._alert.triggered_at.isoformat(),
            "acknowledged_at": self._alert.acknowledged_at.isoformat() if self._alert.acknowledged_at else None,
            "resolved_at": self._alert.resolved_at.isoformat() if self._alert.resolved_at else None,
            "escalated_at": self._alert.escalated_at.isoformat() if self._alert.escalated_at else None,
