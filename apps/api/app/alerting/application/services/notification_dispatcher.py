...
from ...domain.models.notification import (
    NotificationRequest, NotificationChannel, NotificationStatus, 
    NotificationPriority, NotificationTemplate
)
from ...domain.services.notification_strategy import (
    NotificationStrategyRegistry, NotificationPriorityQueue
)
import structlog
...
class NotificationDispatcher:
    ...
    async def send_alert_notifications(self, alert: Alert, rule: AlertRule, notification_type: str = "alert") -> List[NotificationRequest]:
        ...
        try:
            ...
            for request in requests:
                alert.add_notification_sent(f"{request.channel.channel_type}:{request.channel.channel_id}")
    ...
    async def retry_failed_notifications(self, max_retries: int = None) -> int:
        ...
        for request in self._pending_retries:
            ...
            if request.failed_at:
                retry_delay = self._retry_delays[min(request.retry_count, len(self._retry_delays) - 1)]
    ...
    async def _create_notification_request(self, alert: Alert, channel: NotificationChannel, notification_type: str) -> Optional[NotificationRequest]:
        ...
        try:
            ...
            if not channel.supports_priority(priority):
                logger.debug(
                    ...
                    channel_priority_filter=channel.priority_filter.value if channel.priority_filter else None
                )
                return None
    ...
    async def _process_notification_batch(self) -> None:
        ...
        tasks = [
            self._process_single_notification(request, semaphore)
            for request in requests
        ]
        
        await asyncio.gather(*tasks, return_exceptions=True)
    
    async def _process_single_notification(self, request: NotificationRequest, semaphore: asyncio.Semaphore) -> None:
    ...
    async def cleanup(self) -> None:
        ...
        self._failed_notifications = [
            req for req in self._failed_notifications
